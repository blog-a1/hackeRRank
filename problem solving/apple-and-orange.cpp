...
{
    int m=apples.size(),n=oranges.size(),ac=0,oc=0;
    for(int i=0;i<m;i++)
    {
        int as=a+apples[i];
        if(as>=s&&as<=t)    ac++;
...
int main()
{
    string st_temp, ab_temp, mn_temp, apples_temp_temp, oranges_temp_temp;
    
    getline(cin, st_temp);
    vector<string>st = split_string(st_temp);
    int s=stoi(st[0]), t=stoi(st[1]);
    
    getline(cin, ab_temp);
    vector<string>ab = split_string(ab_temp);
    int a=stoi(ab[0]), b=stoi(ab[1]);
    
    getline(cin, mn_temp);
    vector<string>mn = split_string(mn_temp);
    int m=stoi(mn[0]), n=stoi(mn[1]);
